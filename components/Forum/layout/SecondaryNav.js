  ...
  return (
    <nav aria-label="Sidebar" className="sticky top-4 divide-y divide-gray-300">
      <div className="pb-8 space-y-1">
        {navigation.map((item) => (
          <Link key={item.name} href={item.href} passHref aria-current={item.current ? "page" : undefined}>
            <a
              className={`${
      ...
      <div className="pt-10">
        <p className="px-3 text-sm font-medium text-gray-500 uppercase tracking-wider" id="communities-headline">
          Channels
        </p>
        <div className="mt-3 space-y-2" aria-labelledby="communities-headline">
          {props?.channels?.map((item) => (
            <Link key={`channel-${item.slug}`} href={`/forum/channels/${item.slug}/1`} passHref>
              <a
