import Link from "next/link";
import { useRouter } from "next/router";
import React, { useCallback, useEffect, useMemo, useState } from "react";
import classNames from "functions/classNames";
import LoadingNireeka from "../Atoms/LoadingNireeka";
import PaymentMethods from "./PaymentMethods";
import PromoCodeField from "./PromoCodeField";
import {
  PAYMENT_PARTIALLY,
  PAYMENT_PAYPAL,
  PAYMENT_STRIPE,
} from "app/constants/paymentMethods";
import { commafy } from "functions/numbers";
import CheckoutCartItem from "./CheckoutCartItem";
import { useSelector } from "react-redux";
import {
  getShippingAndVatByCountry,
  getShippingAndVatGuestByCountry,
} from "app/api/checkout";
import { toast } from "react-toastify";
import BlurBackdrop from "@/components/Atoms/overlays/BlurBackdrop";
import {XIcon} from "@heroicons/react/outline";
import LocalCartItem from "@/components/Atoms/shopping/LocalCartItem";
import WhiteShadowCard from "@/components/Atoms/cards/WhiteShadowCard";
import CheckoutAggrement from "@/components/CheckOut/CheckoutAggrement";

const paymentMethodsWithPromoCode = [
  PAYMENT_STRIPE,
  PAYMENT_PAYPAL,
  PAYMENT_PARTIALLY,
];

const CheckoutShoppingCart = (props) => {
  const router = useRouter();

  const cartServerItems = useSelector((state) => state?.cartServer?.cartData?.items);
  const cartGuestItems = useSelector((state) => state?.cart?.cartItems);
  const isUserLoggedIn = useSelector((state) => state?.auth.isUserLoggedIn);
  const isGetCartLoading = useSelector((state) => state?.cartServer.isGetCartLoading);
  const cartError = useSelector((state) => state?.cartServer.error);
  const selectedBillingCountry = useSelector((state) => state?.checkout.selectedBillingCountry);


  const cartItems = useMemo(
    () =>
      isUserLoggedIn && cartServerItems?.length
        ? cartServerItems
        : cartGuestItems,
    [cartServerItems, cartGuestItems]
  );

  // Shortening the name, so it's easier to access
  const userCreditValue = props.initialCheckoutData?.credit;
  const initialVat = props.initialCheckoutData?.vat;
  const initialShippingCost = props.initialCheckoutData?.shipping;

  // The result of checking promo code with server;
  // It's an object, with a boolean property, "status", representing if promo code was available and successfully applied
  // A number property, "discount", representing the promo code discount amount;
  const [promoCodeRedeemResult, setPromoCodeRedeemResult] = useState(null);

  // Bill Values
  const [cartTotal, setCartTotal] = useState(0);
  const [shippingFee, setShippingFee] = useState(!!initialShippingCost ? initialShippingCost : 0);
  const [calculatedVat, setCalculatedVat] = useState(!!initialVat ? initialVat : 0);
  const [grandTotal, setGrandTotal] = useState(0);
  const [aggrement, setAggrement] = useState(false);
  const [aggrementError, setAggrementError] = useState(false);
  const [showAggrementErrorModal, setShowAggrementErrorModal] = useState(false);

  // State to control visibility of a loader when fetching shipping and vat fees from server
  const [isShippingNVatLoading, setIsShippingNVatLoading] = useState(false);

  const { setPassValue } = props;

  const handleCreditCheckboxChange = () => {
    if (userCreditValue <= 0) return; // In this case, calling this method is impossible from UI, because there is no checkbox. But just in case :)

    setPassValue((preValue) => {
      return {
        ...preValue,
        isCreditCheckboxSelected: !preValue.isCreditCheckboxSelected,
      };
    });
  };

  const handleGetShippingAndVatFromServer = useCallback(async () => {
    if (!selectedBillingCountry) return;
    try {
      setIsShippingNVatLoading(true);
      let response;
      if (isUserLoggedIn)
        response = await getShippingAndVatByCountry(selectedBillingCountry.id);
      else {
        const items = cartItems?.map(
          (product) =>
            product?.server || {
              product_id: product?.id,
              count: product?.cartQuantity,
            }
        );
        response = await getShippingAndVatGuestByCountry(
          selectedBillingCountry.id,
          items
        );
      }

      setCalculatedVat(response.data.vat);
      setShippingFee(response.data.shipping);
    } catch (error) {
      console.log("Error in message field:", error);
      if (
        error.response &&
        error.response.data &&
        error.response.data.message
      ) {
        toast.error(error.response.data.message);
      } else {
        toast.error(
          "Sorry, we couldn't retrieve shipping and vat fees from server."
        );
      }
    } finally {
      setIsShippingNVatLoading(false);
    }
  }, [selectedBillingCountry]);

  const toogleErrorAggrement = () => {
    if(showAggrementErrorModal) document.getElementsByTagName("body")[0].style.overflow = "auto";
    setShowAggrementErrorModal(prevState => !prevState);
  }

  const showErrorAggrement = () => {
    setAggrementError(true);
    setShowAggrementErrorModal(true)
  }

  const onAccept = () => {
    setAggrementError(true);
    toogleErrorAggrement()
    setAggrement(true)
  }

  useEffect(() => {
    if(aggrement && aggrementError)
      setAggrementError(false)
  }, [aggrement])

  useEffect(() => {
    if (cartItems) handleGetShippingAndVatFromServer();
  }, [handleGetShippingAndVatFromServer, selectedBillingCountry, cartItems]);

  useEffect(() => {
    if (!cartItems) return;

    let cartTotal = 0;
    // Calculating cart total and shipping fee
    for (let i = 0; i < cartItems.length; i++)
      cartTotal +=
        +cartItems[i].total_price ||
        +cartItems[i].price * +cartItems[i].cartQuantity;

    setCartTotal(cartTotal);

    let grandTotalToSet = cartTotal + shippingFee + calculatedVat;

    // Calculating credit
    // If negative, it's automatically calculated,
    // if positive, we look for the checkbox status
    if (userCreditValue < 0) grandTotalToSet -= +userCreditValue;
    else if (props.passValue.isCreditCheckboxSelected)
      grandTotalToSet -= +userCreditValue;

    // Calculating promo code discount
    if (promoCodeRedeemResult && promoCodeRedeemResult.status)
      grandTotalToSet -= +promoCodeRedeemResult.discount;

    setGrandTotal(grandTotalToSet);
    setPassValue((prevValues) => {
      return { ...prevValues, grandTotal: grandTotalToSet };
    });
  }, [
    cartItems,
    selectedBillingCountry,
    userCreditValue,
    props.passValue.isCreditCheckboxSelected,
    promoCodeRedeemResult,
    setPassValue,
    calculatedVat,
    shippingFee,
  ]);

  useEffect(() => {
    if (!paymentMethodsWithPromoCode.includes(props.passValue.payment_type)) {
      setPromoCodeRedeemResult(null);
      setPassValue((prevValues) => ({ ...prevValues, enteredPromoCode: "" }));
    }
  }, [props.passValue.payment_type, setPassValue]);

  useEffect(() => {
    if (router.isReady && (!cartItems || !cartItems?.length)) {
      router.push("/configurator");
    }
  }, []);

  return (
    <div className="flex flex-col py-2 divide-y">
      {/* Shopping Cart */}
      <div className="px-4 sm:py-2 sm:px-6 mb-4">
        <div className="flex items-start justify-between">
          <span className="text-lg font-light text-gray-900">
            Shopping cart
          </span>
        </div>
        {isGetCartLoading ? (
          <div
            className="flex items-center justify-center w-full h-full"
            style={{ height: "30vh" }}
          >
            <LoadingNireeka className="w-12 h-12 border-gray-600" />
          </div>
        ) : cartError || !cartItems ? (
          <span>
            {cartError?.message ||
              (cartItems?.length > 0 ? "An error ocurred." : "Cart is empty")}
          </span>
        ) : (
          <div className="flow-root overflow-auto scrollbar-thumb-zinc-400 hover:scrollbar-thumb-customGrayBorder">
            {cartItems.map((item) => (
              <CheckoutCartItem
                key={item.id + "-" + item.product_id}
                item={item}
              />
            ))}
          </div>
        )}
      </div>

      {/* Billing */}
      <div className="px-4 sm:py-2 sm:px-6 ">
        <div className="flex justify-between py-2 text-sm font-light text-gray-900 sm:text-base mt-3">
          <span>Subtotal</span>
          <span>${commafy(cartTotal)}</span>
        </div>
        {/* Shipping & Delivery */}
        <div className="flex justify-between py-2 text-sm font-light text-gray-900 sm:text-base">
          <span>Shipping &amp; Delivery</span>
          {isShippingNVatLoading ? (
            <LoadingNireeka className="w-4 h-4 border-gray-700" />
          ) : (
            <span>${commafy(shippingFee)}</span>
          )}
        </div>
        {/* Vat & Duties */}
        {calculatedVat !== 0 && (
          <div className="flex justify-between py-2 text-sm font-light text-gray-900 sm:text-base">
            <span className="flex items-center">
              VAT &amp; Duties
              <Link href="/help-center/topic/taxes-duties-vat">
                <a
                  target="_blank"
                  className="mx-1 text-xs text-blue-600 hover:text-blue-400"
                >
                  (Check more details)
                </a>
              </Link>
            </span>
            {isShippingNVatLoading ? (
              <LoadingNireeka className="w-4 h-4 border-gray-700" />
            ) : (
              <span>${commafy(calculatedVat)}</span>
            )}
          </div>
        )}
        {/* Credit */}
        {!!userCreditValue && userCreditValue !== 0 && (
          <div className="flex justify-between py-2 text-sm font-light text-gray-900 sm:text-base">
            <div className="text-sm">
              <legend className="sr-only">Credit</legend>
              <label
                htmlFor="credit"
                className="flex items-center justify-between text-sm font-light text-gray-900 cursor-pointer sm:text-base"
              >
                {userCreditValue > 0 && (
                  <div className="flex items-center h-5 mr-3">
                    <input
                      id="credit"
                      aria-describedby="Credit-description"
                      name="credit"
                      type="checkbox"
                      className="w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500 "
                      checked={props.passValue.isCreditCheckboxSelected}
                      onChange={handleCreditCheckboxChange}
                    />
                  </div>
                )}
                Your Credit
              </label>
            </div>
            <div
              id="comments-description"
              className={classNames(
                "flex justify-between",
                props.passValue.isCreditCheckboxSelected
                  ? "text-nireekaGreen"
                  : "text-gray-400",
                userCreditValue < 0 && "text-red-600"
              )}
            >
              <span>
                {userCreditValue < 0 && "-"}$
                {commafy(Math.abs(userCreditValue))}
              </span>
            </div>
          </div>
        )}
        {/* Promo Code */}
        {promoCodeRedeemResult && promoCodeRedeemResult?.status && (
          <div className="flex justify-between py-2 text-sm font-light sm:text-base text-nireekaGreen">
            <span>Promo Code</span>
            <span>{`-$${commafy(
              Math.abs(promoCodeRedeemResult.discount)
            )}`}</span>
          </div>
        )}

        {/* Grand Total */}
        <div className="flex justify-between py-2 mb-3 mt-5 text-sm font-medium text-gray-900 sm:text-base">
          <span>Grand Total</span>
          {isShippingNVatLoading ? (
            <LoadingNireeka className="w-5 h-5 border-gray-700" />
          ) : (
            <span>${grandTotal > 0 ? commafy(grandTotal) : 0}</span>
          )}
        </div>
      </div>

      {/* Payment Methods */}
      <div className="px-4 sm:py-2 sm:px-6 mb-4">
        <PaymentMethods
          disabled={grandTotal <= 0}
          setPassValue={setPassValue}
          selectedCountryBilling={selectedBillingCountry}
          methods={props.initialCheckoutData?.payments_type || []}
        />
      </div>

      {paymentMethodsWithPromoCode.includes(props.passValue.payment_type) && (
        <div className="px-4 py-6 sm:px-6">
          <PromoCodeField
            setPromoCodeRedeemResult={setPromoCodeRedeemResult}
            setPassValue={setPassValue}
            passValue={props.passValue}
          />
        </div>
      )}
      <div className="px-4 sm:px-6">
        <div className="flex items-start font-light pt-6 pb-3">
          <input
              id="aggrement"
              name="aggrement"
              value="aggrement"
              onChange={(e) => setAggrement(e.target.checked)}
              type="checkbox"
              checked={aggrement}
              className="w-4 h-4 mt-1 rounded cursor-pointer"
          />
          <label className="ml-3">
            <CheckoutAggrement textColor={aggrementError ? 'text-red-600' : 'text-gray-800'} />
          </label>
        </div>
        <div className="flex items-center justify-center mb-6">
          <button
            form="checkoutForm"
            className={classNames(
              "flex items-center justify-center w-full border rounded-lg cursor-pointer py-2 sm:my-2",
              "border-nireekaGreen hover:text-nireekaGreen hover:bg-white text-white bg-nireekaGreen",
              "disabled:border-gray-500 disabled:text-gray-500 disabled:hover:bg-transparent",
              !aggrement ? "!border-gray-500 !text-gray-500 hover:!bg-transparent bg-transparent" : '',
              props.isCheckoutButtonLoading && "cursor-wait"
            )}
            disabled={props.isCheckoutButtonLoading}
            onClick={aggrement ? props.onCheckoutSubmit : showErrorAggrement}
          >
            {props.isCheckoutButtonLoading ? (
              <>
                <LoadingNireeka className="w-4 h-4 mr-2 border-gray-500" />
                <span>Please wait...</span>
              </>
            ) : (
              <span className="text-xs font-light  sm:text-sm">Checkout</span>
            )}
          </button>
        </div>
      </div>
      <BlurBackdrop
          isVisible={showAggrementErrorModal}
          onClose={toogleErrorAggrement}
          backdropColorClass="bg-black/40"
          noXButton
          className="relative w-full mx-auto h-inherit top-7 md:max-w-3xl"
          customStyle={{ width: "calc(100% - 1rem)", overflowY: "hidden" }}
      >
        <WhiteShadowCard>
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-medium text-[14px] text-gray-900">
              Aggrement
            </h2>
            <button
                type="button"
                className="text-gray-400 hover:text-gray-500"
                onClick={toogleErrorAggrement}
            >
              <span className="sr-only">Close</span>
              <XIcon className="w-6 h-6" aria-hidden="true" />
            </button>
          </div>
          <h3 className="my-8 text-xl font-light text-center text-gray-700">
            <CheckoutAggrement />
          </h3>
          <div className="flex flex-col sm:flex-row-reverse items-center space-y-4 sm:space-y-0 justify-between mt-8">
            <button onClick={onAccept} type="button" className="px-4 py-2 text-sm font-medium w-full sm:w-fit text-white bg-green-600 border border-transparent rounded-md shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-50 focus:ring-indigo-500">
              Accept
            </button>
            <button onClick={toogleErrorAggrement} type="button"
                className="px-4 py-2 w-full sm:w-fit text-sm font-medium bg-white text-red-600 border border-red-600 rounded-md shadow-sm hover:bg-red-600 hover:text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-50 focus:ring-indigo-500">
              Deny
            </button>
          </div>
        </WhiteShadowCard>
      </BlurBackdrop>
    </div>
  );
};

export default React.memo(CheckoutShoppingCart);
